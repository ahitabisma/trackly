...
func ParseIDFromURL(urlPath string) (int, error) {
	// path.Base mengambil elemen terakhir dari sebuah path
	...
	if err != nil {
		return 0, errors.New("invalid ID format, must be an integer")
	}

	// Validasi jika ID tidak boleh nol atau negatif
	if id <= 0 {
...
func ParseCompanyCSV(reader io.Reader) ([]CompanyImportRequest, error) {
	csvReader := csv.NewReader(reader)
	csvReader.FieldsPerRecord = -1 // Allow variable number of fields

	var records []CompanyImportRequest
	...
	for {
		...
		records = append(records, CompanyImportRequest{
			Kode:              kode,
