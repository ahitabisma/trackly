...
const NA = (v: any) => (v === null || v === undefined ? 'N/A' : typeof v === 'number' ? v.toLocaleString('id-ID') : v)
const NAFmt = (v: any) => {
...
const { query, suggestions, loading: tickerLoading, fetchAll } = useTickerSearch()
const { result, loading: analisisLoading, error: analisisError, run: runAnalisis, clear: clearResult } = useAnalisis()
...
const signalBadge = computed(() => {
    ...
    if (o === 'bullish') return { label: 'BULLISH', class: 'bg-green-100 text-green-800 border-green-300' }
    if (o === 'bearish') return { label: 'BEARISH', class: 'bg-red-100 text-red-800 border-red-300' }
    return { label: 'NETRAL', class: 'bg-gray-100 text-gray-600 border-gray-300' }
})

function drawChart(ohlcv: { date: string; open: number; high: number; low: number; close: number; volume: number }[]) {
    ...
    const data = ohlcv.map(d => ({ ...d, parsed: parseDate(d.date)! })).filter(d => d.parsed)
    ...
    overlay.on('mousemove', (event: MouseEvent) => {
        ...
        const d1 = data[i]
        if (!d0 || !d1) return
        const d = x0.getTime() - d0.parsed.getTime() > d1.parsed.getTime() - x0.getTime() ? d1 : d0
        tooltip
            .style('display', 'block')
            .html(`
...
watch(() => result.value?.ohlcv, (ohlcv) => {
    if (ohlcv?.length) {
...
<template>
    <main class="min-h-screen bg-cream text-ink font-sans antialiased overflow-x-hidden pt-20 pb-24">
        <div class="max-w-5xl mx-auto px-5 sm:px-8">
            <div class="fu mb-8">
                <h1 class="font-serif text-4xl text-ink" style="letter-spacing:-1px">Analisis</h1>
                <p class="font-mono text-xs text-muted mt-1 uppercase tracking-wider">Cari dan analisis pergerakan harga
                    saham</p>
            </div>

            <!-- Search + Date + Submit -->
            <div class="fu1 bg-card border-2 border-ink rounded-2xl p-5 sm:p-6 mb-8" style="box-shadow:4px 4px 0 #1a1612">
                <div class="flex flex-col sm:flex-row gap-4 items-end">
                    <div class="flex-1 w-full relative">
                        <label class="font-mono text-xs text-muted uppercase tracking-wider block mb-1.5">Ticker</label>
                        <div v-if="selectedTicker" class="search-input flex items-center justify-between cursor-default">
                            <span>
                                <span class="font-mono text-sm font-bold text-bluedk bg-bluebg px-2 py-0.5 rounded-md mr-2">{{ selectedTicker.kode }}</span>
                                <span class="text-sm text-ink2">{{ selectedTicker.nama }}</span>
                            </span>
                            <button class="font-mono text-xs text-muted hover:text-ink ml-2" @click="clearTicker">✕</button>
                        </div>
                        <input v-else v-model="query" class="search-input" type="text"
                        ...
                        <div v-if="showDropdown"
                            ...
                            <div v-if="tickerLoading" class="px-4 py-4 text-center">
                                <span class="font-mono text-xs text-muted">Memuat daftar ticker...</span>
                            </div>
                            <div v-else-if="suggestions.length">
                                <div v-for="s in suggestions" :key="s.kode"
                    ...
                    <div class="w-full sm:w-40">
                        <label class="font-mono text-xs text-muted uppercase tracking-wider block mb-1.5">Dari</label>
                        <input v-model="dateStart" type="date"
                            class="neo-input" />
                    </div>

                    <div class="w-full sm:w-40">
                        <label class="font-mono text-xs text-muted uppercase tracking-wider block mb-1.5">Sampai</label>
                        <input v-model="dateEnd" type="date"
                            class="neo-input" />
                    </div>

                    <div class="w-full sm:w-auto">
                        <button class="neo-btn-primary whitespace-nowrap" :disabled="!selectedTicker || analisisLoading"
                            @click="submit">
                            <span v-if="analisisLoading" class="inline-block w-4 h-4 border-2 border-cream border-t-transparent rounded-full animate-spin"></span>
            ...
            <div v-if="analisisLoading && !result" class="fu2 text-center py-14">
                ...
                <p class="font-mono text-sm text-muted">Mengambil data dan menghitung indikator...</p>
            </div>

            <!-- Results -->
            <div v-if="result" class="space-y-6">
                <!-- Snapshot Card -->
                <div v-if="result.snapshot" class="fu2 bg-card border-2 border-ink rounded-2xl p-5 sm:p-6"
                    style="box-shadow:4px 4px 0 #1a1612">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div>
                            ...
                            <div class="font-serif text-2xl mt-0.5">{{ formatPrice(result.snapshot.price) }}</div>
                        </div>
                        <div>
                            <div class="font-mono text-xs text-muted uppercase">52w High</div>
                            <div class="font-serif text-2xl mt-0.5">{{ formatPrice(result.snapshot.high52) }}</div>
                        </div>
                        <div>
                            <div class="font-mono text-xs text-muted uppercase">52w Low</div>
                            <div class="font-serif text-2xl mt-0.5">{{ formatPrice(result.snapshot.low52) }}</div>
                        </div>
                        <div>
                            <div class="font-mono text-xs text-muted uppercase">Volume</div>
                            <div class="font-serif text-2xl mt-0.5">{{ formatCompact(result.snapshot.volume) }}</div>
                        </div>
                        <div>
                            <div class="font-mono text-xs text-muted uppercase">Market Cap</div>
                            <div class="font-serif text-2xl mt-0.5">{{ formatCompact(result.snapshot.marketcap) }}</div>
                        </div>
                        <div>
                            <div class="font-mono text-xs text-muted uppercase">PE</div>
                            <div class="font-serif text-2xl mt-0.5">{{ NA(result.snapshot.pe) }}</div>
                        </div>
                        <div>
                            <div class="font-mono text-xs text-muted uppercase">EPS</div>
                            <div class="font-serif text-2xl mt-0.5">{{ formatPrice(result.snapshot.eps) }}</div>
                        </div>
                        <div>
                            <div class="font-mono text-xs text-muted uppercase">Currency</div>
                            <div class="font-serif text-2xl mt-0.5">{{ result.snapshot.currency || 'N/A' }}</div>
                        </div>
                    </div>
                </div>

                <!-- D3 Candlestick Chart -->
                <div v-if="result.ohlcv?.length" class="fu2 bg-card border-2 border-ink rounded-2xl p-5 sm:p-6"
                    style="box-shadow:4px 4px 0 #1a1612">
                    <h2 class="font-mono text-xs text-muted uppercase tracking-wider mb-4">Grafik Harga Historis</h2>
                    <svg ref="chartSvg" class="w-full" :style="{ height: '400px' }" style="overflow:visible"></svg>
                </div>

                <!-- Python Multi-panel Chart -->
                <div v-if="result.chart_image" class="fu2 bg-card border-2 border-ink rounded-2xl p-5 sm:p-6"
                    style="box-shadow:4px 4px 0 #1a1612">
                    <h2 class="font-mono text-xs text-muted uppercase tracking-wider mb-4">Chart Teknikal Lengkap</h2>
                    <img :src="'data:image/png;base64,' + result.chart_image" class="w-full h-auto" alt="Technical Analysis Chart" />
                </div>
                ...
                <div v-if="result.signal" class="fu3 bg-card border-2 border-ink rounded-2xl p-5 sm:p-6"
                    ...
                    <div v-if="signalExpanded" class="mt-4 space-y-1">
                        <div v-for="b in result.signal.breakdown" :key="b.indicator"
                            class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-mono"
                            :class="b.signal === 'bullish' ? 'bg-green-50' : b.signal === 'bearish' ? 'bg-red-50' : 'bg-gray-50'">
                            <span class="font-bold w-28 shrink-0">{{ b.indicator }}</span>
                            <span class="text-xs uppercase font-bold"
                                :class="b.signal === 'bullish' ? 'text-green-700' : b.signal === 'bearish' ? 'text-red-700' : 'text-gray-500'">
                                {{ b.signal }}
                            </span>
                ...
                <!-- Indicators Grid -->
                <div v-if="result.indicators" class="fu3 bg-card border-2 border-ink rounded-2xl p-5 sm:p-6"
                    style="box-shadow:4px 4px 0 #1a1612">
                    <h2 class="font-mono text-xs text-muted uppercase tracking-wider mb-4">Indikator Teknikal</h2>
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
                        <div><div class="font-mono text-xs text-muted uppercase">SMA20</div><div class="font-mono text-lg font-bold mt-0.5">{{ formatPrice(result.indicators.sma20) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">SMA50</div><div class="font-mono text-lg font-bold mt-0.5">{{ formatPrice(result.indicators.sma50) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">SMA200</div><div class="font-mono text-lg font-bold mt-0.5">{{ NAFmt(result.indicators.sma200) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">EMA20</div><div class="font-mono text-lg font-bold mt-0.5">{{ formatPrice(result.indicators.ema20) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">RSI</div><div class="font-mono text-lg font-bold mt-0.5">{{ NAFmt(result.indicators.rsi?.toFixed(2)) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">MACD</div><div class="font-mono text-lg font-bold mt-0.5">{{ NAFmt(result.indicators.macd?.toFixed(2)) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">ADX</div><div class="font-mono text-lg font-bold mt-0.5">{{ NAFmt(result.indicators.adx?.toFixed(2)) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">ATR</div><div class="font-mono text-lg font-bold mt-0.5">{{ NAFmt(result.indicators.atr) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">Support</div><div class="font-mono text-lg font-bold mt-0.5">{{ formatPrice(result.indicators.support) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">Resistance</div><div class="font-mono text-lg font-bold mt-0.5">{{ formatPrice(result.indicators.resistance) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">BB Width</div><div class="font-mono text-lg font-bold mt-0.5">{{ NAFmt(result.indicators.bb_width?.toFixed(4)) }}</div></div>
                        <div><div class="font-mono text-xs text-muted uppercase">Volume Spike</div><div class="font-mono text-lg font-bold mt-0.5">{{ NAFmt(result.indicators.volume_spike) }}</div></div>
                    </div>
                </div>

                <!-- Trading Plan -->
                <div v-if="result.trading_plan" class="fu3 bg-card border-2 border-ink rounded-2xl p-5 sm:p-6"
                    style="box-shadow:4px 4px 0 #1a1612">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-5">
                        <div>
                            ...
                            <div class="font-mono text-lg font-bold mt-0.5 uppercase"
                                :class="result.trading_plan.bias === 'buy' ? 'text-green-700' : result.trading_plan.bias === 'sell' ? 'text-red-700' : 'text-gray-500'">
                                {{ result.trading_plan.bias }}
                            </div>
                        </div>
                        <div>
                            <div class="font-mono text-xs text-muted uppercase">Entry</div>
                            <div class="font-mono text-lg font-bold mt-0.5">{{ result.trading_plan.entry_price !== null && result.trading_plan.entry_price !== undefined ? formatPrice(result.trading_plan.entry_price) : 'N/A' }}</div>
                        </div>
                        <div>
                            <div class="font-mono text-xs text-muted uppercase">Stop Loss</div>
                            <div class="font-mono text-lg font-bold mt-0.5 text-red-600">{{ result.trading_plan.stop_loss !== null && result.trading_plan.stop_loss !== undefined ? formatPrice(result.trading_plan.stop_loss) : 'N/A' }}</div>
                        </div>
                        <div>
                            <div class="font-mono text-xs text-muted uppercase">Ukuran Posisi</div>
                            <div class="font-mono text-lg font-bold mt-0.5">{{ result.trading_plan.suggested_position_size_pct.toFixed(2) }}%</div>
                        </div>
                    </div>
                    ...
                    <div v-if="result.trading_plan.targets?.length" class="mb-4">
                        ...
                        <table class="w-full text-left font-mono text-xs data-table border-2 border-bdr rounded-xl overflow-hidden">
                            ...
                            <tbody>
                                <tr v-for="t in result.trading_plan.targets" :key="t.level"
                                    class="border-t border-bdr/50">
                                    <td class="py-2 px-3 font-bold">TP{{ t.level }}</td>
                                    <td class="py-2 px-3">{{ formatPrice(t.price) }}</td>
                                    <td class="py-2 px-3 text-green-700 font-bold">{{ t.rr_ratio.toFixed(2) }}R</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                        ...
                        <p class="font-mono text-xs text-yellow-800">{{ result.trading_plan.invalidation_note }}</p>
                    </div>
                    ...
                    <div class="p-3 bg-gray-100 border border-gray-200 rounded-lg">
                        ...
                        <p class="font-mono text-xs text-gray-600 leading-relaxed">{{ result.trading_plan.disclaimer }}</p>
                    </div>
                </div>
            ...
            <!-- Empty State -->
            <div v-if="!result && !analisisLoading && !analisisError"
                class="text-center py-20">
                <p class="font-mono text-sm text-muted">Pilih ticker dan rentang tanggal, lalu klik Analisis untuk memulai.</p>
            </div>
        </div>
    </main>
</template>
