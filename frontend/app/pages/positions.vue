      ...
      <div v-if="positionAnalysis" class="fu3 space-y-4">
        ...
        >
          ...
          <div v-else-if="pnl" class="space-y-4">
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div>
                ...
                <div
                  ...
                  :class="
                    pnl.unrealized_pnl >= 0 ? 'text-green-700' : 'text-red-700'
                  "
                >
            ...
            <div
              v-if="pnl.suggested_exit_price || pnl.suggested_stop_price"
            ...
            >
              <div v-if="pnl.suggested_exit_price">
                ...
                <div class="font-mono text-sm font-bold mt-0.5">
                  {{ formatNum(pnl.suggested_exit_price) }}
                </div>
              </div>
              <div v-if="pnl.suggested_stop_price">
                ...
                <div class="font-mono text-sm font-bold mt-0.5 text-red-600">
                  {{ formatNum(pnl.suggested_stop_price) }}
                </div>
              </div>
            </div>
        ...
        >
          <button
            ...
            @click="requestAiInsight(positionAnalysis.ticker)"
            :disabled="aiLoading"
          >
            <span
              v-if="aiLoading"
          ...
          <div v-if="aiInsight" class="mt-3">
            <div class="flex items-center justify-between mb-3">
              ...
              <button
                class="font-mono text-xs text-muted hover:text-ink underline max-w-full"
          ...
          <div v-if="aiError" class="mt-3 font-mono text-xs text-red-600">
            {{ aiError }}
          </div>
        </div>
        ...
        >
          <h2
            class="font-mono text-xs text-muted uppercase tracking-wider mb-3"
          >
            Sinyal Teknikal
          </h2>
          <div class="flex items-center gap-3 mb-3">
            <span
              ...
              :class="
                positionAnalysis.signal?.overall === 'bullish'
                  ? 'bg-green-100 text-green-800 border-green-300'
                  : positionAnalysis.signal?.overall === 'bearish'
            ...
            <span class="font-mono text-xs text-muted"
              >Skor: {{ positionAnalysis.signal?.score?.toFixed(2) }}</span
            >
            <span
              v-if="positionAnalysis.signal?.confidence"
              class="font-mono text-xs px-2 py-0.5 rounded border"
              :class="
                positionAnalysis.signal.confidence === 'high'
                  ? 'bg-green-50 text-green-700 border-green-200'
                  : positionAnalysis.signal.confidence === 'medium'
          ...
          <details
            class="group"
            v-if="positionAnalysis.signal?.breakdown?.length"
          >
            <summary
              class="font-mono text-xs text-muted cursor-pointer hover:text-ink select-none list-none flex items-center gap-2"
            >
              <span class="text-xs transition-transform group-open:rotate-90"
                >▶</span
              >
              Detail sinyal ({{
                positionAnalysis.signal.breakdown.length
              }}
              indikator)
            </summary>
          ...
          <div class="grid grid-cols-3 sm:grid-cols-4 gap-4">
            <div>
              ...
              <div class="font-mono text-sm font-bold mt-0.5">
                {{
                  formatNum(
                    positionAnalysis.indicators._close?.[
                      positionAnalysis.indicators._close.length - 1
                    ] || 0,
                  )
                }}
              </div>
            ...
            <div>
              ...
              <div class="font-mono text-sm font-bold mt-0.5">
                {{ formatNum(positionAnalysis.position.average_buy_price) }}
              </div>
            </div>
            <div>
              <div class="font-mono text-[10px] text-muted uppercase">RSI</div>
              <div class="font-mono text-sm font-bold mt-0.5">
                {{ positionAnalysis.indicators.rsi?.toFixed(2) || "N/A" }}
              </div>
            </div>
            <div>
              <div class="font-mono text-[10px] text-muted uppercase">MACD</div>
              <div class="font-mono text-sm font-bold mt-0.5">
                {{ positionAnalysis.indicators.macd?.toFixed(2) || "N/A" }}
              </div>
            </div>
          </div>
        </div>
